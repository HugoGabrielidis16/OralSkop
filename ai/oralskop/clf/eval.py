...
Handles both checkpoint kinds (the checkpoint's own class list / preprocessing is
authoritative — it matches the trained head):
...
def _resolve_weights(cfg: dict, cli: str | None) -> Path:
    ...
    run_dir = AI_ROOT / cfg.get("out_dir", "runs/clf") / cfg.get("name", "clf_coarse")
    if (run_dir / "best.pt").exists():
        return run_dir / "best.pt"
    if run_dir.exists():
        return run_dir
    raise SystemExit(f"No --weights given and nothing at {run_dir}. Pass --weights <path>.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    y_true, y_score = collect_scores(model, loader, device, desc="test", leave=True,
                                     use_amp=use_amp, amp_dtype=amp_dtype, is_hf=is_hf)
