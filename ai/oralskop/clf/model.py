"""Model factory for the multi-label classifier.

Two families, one interface (``build_model`` returns ``(model, preprocess, is_hf)``):

* **torchvision** backbones (``convnext_tiny/small/base``, ``resnet50``,
  ``efficientnet_v2_s``) — full fine-tune, ImageNet normalization.
* **foundation models** (HuggingFace, e.g. ``dinov2_large``) fine-tuned with **QLoRA**:
  a 4-bit NF4 base (frozen) + LoRA adapters + a trainable multi-label head, for a small
  memory footprint. Normalization/size come from the model's own image processor.

Outputs are raw logits — the loss (``BCEWithLogitsLoss``) applies the sigmoid; inference
thresholds ``sigmoid(logits)``. ``forward_logits`` (in ``train.py``) unifies the call.
"""
...
# torchvision arch name -> (constructor, default-weights enum)
_TV_ARCHS = {
...
def build_classifier(num_classes: int, *, arch: str = "convnext_tiny", pretrained: bool = True):
    """Build a torchvision multi-label classifier with a fresh ``num_classes`` head."""
    if arch not in _TV_ARCHS:
        raise ValueError(f"Unknown torchvision arch {arch!r}. Options: {', '.join(sorted(_TV_ARCHS))}.")
    ctor, weights = _TV_ARCHS[arch]
...
def build_foundation_model(
    ...
    *,
    ...
    if lora:
        ...
        targets = lora_target_modules or _DEFAULT_LORA_TARGETS
        peft_cfg = LoraConfig(
            r=int(lora_r), lora_alpha=int(lora_alpha), lora_dropout=float(lora_dropout),
            target_modules=list(targets), bias="none", modules_to_save=["classifier"],
        )
...
def build_model(num_classes: int, cfg: dict, *, compute_dtype: torch.dtype = torch.bfloat16):
    ...
    preprocess = {"mean", "std", "imgsz"}. Torchvision archs use ImageNet stats and the
    config's ``imgsz`` (default 224); foundation models use their processor's values.
    """
    ...
    if hf_id is not None:
        ...
        model, preprocess, is_hf = build_foundation_model(
            num_classes, hf_id,
            quantize=cfg.get("quantize", "4bit"),
