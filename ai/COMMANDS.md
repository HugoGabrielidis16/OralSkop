...
## 3e. Multi-label classification (manifest dataset, S3)
...
For a stronger backbone at a small memory footprint, the same path can fine-tune a
HuggingFace foundation model with **QLoRA**: a 4-bit NF4 base (frozen) + LoRA adapters +
a trainable multi-label head, an **8-bit paged optimizer**, **gradient checkpointing**,
and **gradient accumulation**. Tuned for an A10G (~24 GB); needs CUDA (bitsandbytes).
...
from the model's image processor. bf16 is auto-selected on A10/A100 (fp16 on T4). Saves
LoRA **`adapter_best/` / `adapter_last/` + `meta.json`** (not a full `.pt`). The
torchvision archs now also include `convnext_small` / `convnext_base`.
